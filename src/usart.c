...
void initLeuart(LEUART_TypeDef *leuart, u32 baudrate, u8 powerflag)
{
    ...
    else //LEUART1
    {
        ...
        {
            GPIO_PinModeSet(gpioPortC, 6, gpioModeInputPull, 0);  //    org  gpioModeInput 
            GPIO_PinModeSet(gpioPortC, 7, gpioModeInputPull, 0);    /* Pull direction is set to pull-up */   // org gpioModeInput
        }
    }
...
void TaskUsbSend(void)
{
    ...
    if( (0 == usbbuflen) || (GsmSta.charging != BATTERY_CHARGE))
    {
        return;
    }
