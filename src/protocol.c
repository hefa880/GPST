...
void SendPosition ( u8 intime )
{
    ...
    if ( GsmSta.voltage < LOW_VOLTAGE  || cnt++ < 10 || 
        MASK_POWER_STATUS_OFF == GsmSta.gsm_p )
    {
        return;
    }
    ...
    if(  timer.time[0] < 0x17 )
    {
        if( timer.counter - GsmSta.BasicPositionInter  > 600 )
        {
            ...
            else
            {
                ...
                GsmSta.gps_p = MASK_POWER_STATUS_WAKEUP;    // wake up
            }
    ...
    if ( fristTimeGetTime == 0 && timer.time[0] > 0x16 )
    {
        fristTimeGetTime = 1;
        timecounter = 0;
        GsmSta.BasicPositionInter = timer.counter  - 30;
        packet_current = 0;

       // return;
    }

    if(( ( timer.counter - GsmSta.BasicPositionInter ) >= ( inter - 30) ) /*|| 1 == fristTimeGetTime */ )
    {
        if(bvkstrGpsData.Latitude != INVALID_LON_LAT  )
        { 
            if( GsmSta.gps_p != MASK_POWER_STATUS_SLEEP  && GsmSta.askGpsTimes > 5 )
            {
                GsmSta.gps_p = MASK_POWER_STATUS_SLEEP; // 休眠
                GsmSta.askGpsTimes = 0;
                 myprintf("SendPosition: GPS sleep.\r\n");
            }
        ...
        {
            if(GsmSta.gps_p == MASK_POWER_STATUS_OFF) 
            {
                GsmSta.gps_p = MASK_POWER_STATUS_REST;
                myprintf("SendPosition: GPS rest.\r\n");
            }
            else if(GsmSta.gps_p  == MASK_POWER_STATUS_SLEEP)
            {
                GsmSta.gps_p  = MASK_POWER_STATUS_WAKEUP; // 唤醒
                myprintf("SendPosition: GPS wake up.\r\n");
            }
        }

        if(GsmSta.Latitude == INVALID_LON_LAT )
        {
    ...
    switch ( stu )
    {
        case 0:/*时间到达时候更新时间,然后请求经纬度*/
                 ...
                 ( intime != SEND_POSITION_IN_TIME ) /**/ )
            {
                ...
                if ( /* (unfixedtime<30)&&*/ (bvkstrGpsData.Latitude != INVALID_LON_LAT) /*&& bvkstrGpsData.Latitude != bvkstrGpsData.longitude*/ )
                {
                ...
                else if ( GsmSta.Latitude != INVALID_LON_LAT /*&& GsmSta.longitude != GsmSta.Latitude*/)
                {
                    temInter = ProcessPositionJump(GsmSta.longitude, GsmSta.Latitude, 3, timer.counter);
                      if( 0xF0 == temInter )
                    {
                        if( GpsControlStu.GpsUnfixedTime > FIX_FIRST_TIME_SEC )
                        {
                           // GpsControlStu.GpsLocaltionSatus = GPS_LOCALTION_INSIDE; // 认为在室内
                        }
                        else
                        {
                          //   GpsControlStu.GpsLocaltionSatus = GPS_LOCALTION_OUTSIDE; // 认为在室外
                        }
...
GPS:
            ...
            */
            ...
            //         if ( /*(bvkstrGpsData.longitude==0)||*/ ( bvkstrGpsData.Latitude == INVALID_LON_LAT ) )
            //         {
            //            stu = 0;
        ...
        case 2:
            ...
            if ( /*(unfixedtime<30)&&*/ ( bvkstrGpsData.Latitude != INVALID_LON_LAT ) )
            {
                GsmSta.askm2m = 0;
                GsmSta.askm2malerag = 0;
                GsmSta.Askmsmback = 0;
            ...
            else if ( GsmSta.Latitude != INVALID_LON_LAT )
            {
                GsmSta.askm2m = 0;
                GsmSta.askm2malerag = 0;
                GsmSta.Askmsmback = 0;
...
LBS:
            ...
            if ( ( GsmSta.Latitude == INVALID_LON_LAT ) || ( GsmSta.longitude == 0 )  )
            {
                stu = 0;
                GsmSta.askm2m = 1;
            #ifdef USE_PRINTF
...
myprintf ( " GPS#Lat %d, Lon:%d LBS#Lat %d,Lon:%d   SN=%d\r\n",
             bvkstrGpsData.Latitude, bvkstrGpsData.longitude, GsmSta.Latitude, GsmSta.longitude,
             packet_current
           );
}
